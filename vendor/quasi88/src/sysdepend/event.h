 ...
 *      エミュ／メニュー(フルメニュー、ポーズetc)の切替時に呼び出される。
 ...
 *		** 現在、未使用 **
 *      キーレイアウトの変更／復元をする際に、呼び出される。
 *      どのように変更するかは、システム依存とする。
 *      変更できないならば、 event_keylayout_change() の戻り値を 偽 にする。
 *
 * void event_get_mouse_pos(int *x, int *y)
 *      現在のマウスの絶対座標をセットする。
 *      絶対座標の概念がない場合は、画面中央座標などをセットしておけばよい。
