...
static const T_CONFIG_TABLE xmame_options[] = {
	...
	{ 363, "samplefreq",	X_INT,	&options.samplerate,		8000,	44100,			0,				OPT_SAVE	},
...
void xmame_config_show_option(FILE *fp)
{
	fprintf(
		...
		"    -pcgvol <level>         Set PCG    level to <level> %%, (0 - 100) [60]\n"
...
static Boolean isFullSoundStream(int dataNum)
{
	...
	if (soundIn < sSoundOut) {
		if ((soundIn + dataNum) <= sSoundOut) {
			updateOk = true;
		}
	} else if ((soundIn + dataNum) > sSoundStreamFrames) {
		if ((soundIn + dataNum - sSoundStreamFrames) <= sSoundOut) {
			updateOk = true;
		}
	} else {
		updateOk = true;
...
int osd_update_audio_stream(INT16 *buffer)
{
...
#if 1
	{
		...
		if (isFullSoundStream(sound_samples_per_frame * CHANNELS)) {
			return sound_samples_per_frame;
		}
