...
/*----------------------------------------------------------------------
 * Queued Input API (for headless automation)
 *----------------------------------------------------------------------*/
/*
 * quasi88_keyinject - Queue a key sequence for later consumption
 * @sequence: ASCII string with escapes (e.g., "HELLO<CR>")
 *
 * Returns:
 *   0  Success (queued)
 ...
 *
 * Legal modes: MONITOR, EXEC, PAUSE
 * Illegal modes: MENU, QUIT
 *
 * Atomic semantics:
 *   - The entire sequence is enqueued or nothing is enqueued
 *   - On overflow, queue state is preserved (no partial enqueue)
 *   - Each character/key requires 2 queue slots (press + release)
 *
 * Queue lifecycle:
 *   - PRESERVED: Across MONITOR -> EXEC transition
 *   - PRESERVED: Across EXEC -> MONITOR -> EXEC cycle
 *   - FLUSHED: On emulator reset (quasi88_reset)
 *   - FLUSHED: On shutdown/exit
 *   - FLUSHED: On mode transition to MENU or QUIT
 *
 * Drain semantics:
 *   - One key event (press OR release) drained per keyboard_update() call
 *   - Drain only occurs in EXEC mode
 *   - Empty queue returns FALSE, no modification to key state
 */
 ...
 *
 * Called during EXEC mode to consume queued keys.
 * Returns TRUE if a key was drained, FALSE if queue empty.
 *
 * Sets code_out and press_flag_out to the drained key event.
 *
 * Drain unit: One key event per call (press OR release, not both)
 * This ensures deterministic ordering and prevents burst injection.
 *
 ...
 *
 * Called on:
 *   - Emulator reset (emu_reset path)
 *   - Shutdown/exit paths
 *   - Mode transition to MENU or QUIT
 *
 * This clears all pending keys and resets queue pointers.
 */
