...
byte	main_fetch( word addr )
{
  ...
  /* Phase 3: BASIC interpreter state observation hook
   * When PC reaches configured observation addresses, update exec_state.
   * This provides OCR-free visibility into BASIC interpreter state.
   */
...
void	pc88main_init( int init )
{
  ...
  /* Phase 3: Initialize BASIC interpreter state observation hook addresses
   * These addresses are specific to N88-BASIC V2 ROM.
   * - READY prompt: PC range around the main command loop (after "Ok" message)
   * - INPUT_WAIT: PC when INPUT statement is waiting (keyboard poll)
   * - ERROR: PC when error handler is invoked
   *
   * For N88-BASIC V2:
   * - READY: Command return loop after line execution (0x0320-0x04e0 range)
   * - INPUT_WAIT: INPUT routine (0x0600-0x0700 range) - needs refinement
   * - ERROR: Error handler entry at 0x047b
   *
   * NOTE: These addresses may vary by ROM version. Adjust accordingly.
   */
  quasi88_set_rom_hook_addresses(
    0x0320,    /* ready_pc_low - after "Ok" at 0x0326 */
    0x04e0,    /* ready_pc_high - before encode routine at 0x04e2 */
    0x0600,    /* input_wait_pc - INPUT statement handler (approximate) */
    0x047b     /* error_pc - error handler entry */
  );
