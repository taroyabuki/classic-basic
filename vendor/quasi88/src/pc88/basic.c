...
#define BASIC_ENCODE_LOOP_PER_CHAR	50000L
...
/*------------------------------------------------------
 * 長い数値リテラルを含む行はエンコード完了まで時間がかかる。
 * 固定ステップ数だと valid な行でも途中で打ち切られるため、
 * 行長に応じて仮想 CPU の実行予算を増やす。
 *------------------------------------------------------*/
static long basic_encode_loop_budget(const char *line)
{
	size_t line_len = strlen(line);

	if (line_len == 0) {
		return BASIC_MAX_LOOP;
	}
	if (line_len > (size_t)((LONG_MAX - BASIC_MAX_LOOP) / BASIC_ENCODE_LOOP_PER_CHAR)) {
		return LONG_MAX;
	}
	return BASIC_MAX_LOOP + (long)line_len * BASIC_ENCODE_LOOP_PER_CHAR;
}
...
static void pseudo_mem_write(word addr, byte data)
{
	if (addr < 0x8000) {
		...
		write_pseudo_mem_8000_83ff[addr & 0x03ff] = data;
	} else {
		pseudo_ram[addr] = data;
	}
}
...
static byte pseudo_mem_read(word addr)
{
	if (addr < 0x8000) {
		...
		return read_pseudo_mem_8000_83ff[addr & 0x03ff];
	} else {
		return pseudo_ram[addr];
	}
}
 ...
 *
 *------------------------------------------------------*/
void pseudo_intr_update(void)
{
	pseudo_z80_cpu.skip_intr_chk = TRUE;
}
int pseudo_intr_ack(void)
{
	return -1;
}
...
int basic_encode_list(FILE *fp)
{
	...
	while (fgets(buf, BASIC_MAX_LINE, fp) != NULL) {
		...
		loop_budget = basic_encode_loop_budget(ptr);

		/* 仮想プロセス開始 */
		for (loop = 0; loop < loop_budget; loop++) {
			z80_emu(&pseudo_z80_cpu, 1);
			if (pseudo_z80_cpu.PC.W == encode_end_pc) {
				break;
			}
...
int basic_decode_list(FILE *fp)
{
	...
	while (line_top_addr < basic_end_addr) {
		...
		for (loop = 0; loop < BASIC_MAX_LOOP; loop++) {
			z80_emu(&pseudo_z80_cpu, 1);
			if (pseudo_z80_cpu.PC.W == decode_end_pc) {
				break;
			}
		...
		for (i = 0; i < BASIC_MAX_LINE - 1; i++) {
			if (READ_BYTE(pseudo_ram, basic_conv_buffer_addr + i) == 0x00) {
				break;
			}
...
int basic_load_intermediate_code(FILE *fp)
{
	...
	size = (int)fread(&main_ram[basic_buffer_addr - 1], 1, basic_buffer_size + 1, fp);
...
int basic_save_intermediate_code(FILE *fp)
{
	...
	wsize = (int)fwrite(&main_ram[basic_top_addr - 1], 1, size, fp);
	if (wsize < size) {
