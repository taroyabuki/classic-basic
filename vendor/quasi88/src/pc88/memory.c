...
byte (*kanji_rom)[65536][2];			/* 漢字 ROM  [2][65536][2] */
...
/* メモリ確保開始 */
static void mem_alloc_start(const char *msg)
{
	if (verbose_proc) {
		printf("%s", msg);
	}
...
/* メモリ確保完了(偽で失敗) */
static int mem_alloc_finish(void)
{
	if (verbose_proc) {
	...
	if (dir) {
		for (i = 0; filelist[i] ; i++) {
			...
			if ((fp = osd_fopen(FTYPE_ROM, buf, "rb"))) {
				...
				load_size = (int)osd_fread(ptr, sizeof(byte), size, fp);
				osd_fclose(fp);
...
static int load_compat_rom(byte *ptr, long pos, int size, OSD_FILE *fp)
{
	...
	if (fp) {
		if (osd_fseek(fp, pos, SEEK_SET) == 0) {
			...
			load_size = (int)osd_fread(ptr, 1, size, fp);
...
int memory_allocate(void)
{
	...
	if (file_compatrom == NULL) {
		...
		if (fp) {
			...
			if (load_compat_rom_success == FALSE) {
				/* ここ迄で失敗があれば終了 */
				if (verbose_proc) {
					printf("FAILED\n");
				}
 ...
 *
 *----------------------------------------------------------------------*/
int quasi88_now_use_pcg(void)
{
	return use_pcg;
}

void quasi88_set_use_pcg(int use)
{
	use_pcg = use;
