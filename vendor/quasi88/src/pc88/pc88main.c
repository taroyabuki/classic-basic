...
static void pcg_out_addr_high(byte addr)
{
	...
	if (addr & 0x10) {
		...
		if (addr & 0x20) {
			/* copy */
			src = font_mem[ 0x400 + (pcg_addr & 0x3ff) ];
		} else             {
			/* store */
			src = pcg_data;
		}
...
static void pcg_sound_init(void)
{
	int i;

	memset(&pcg_sound_wk, 0, sizeof(pcg_sound_wk));

	pcg_sound_wk.gate = 0xc8;
	for (i = 0; i < 3; i++) {
		pcg_sound_wk.ch[i].access = 3;
	}
}

static void pcg_sound_out_gate(byte data)
{
	pcg_sound_wk.gate = data;
	xmame_dev_pcg_out_gate(data);
}

static void pcg_sound_out_counter_sub(int ch, byte data)
{
	pcg_sound_wk.ch[ch].counter[ pcg_sound_wk.ch[ch].access_step ] = data;

	if (pcg_sound_wk.ch[ch].access == 3) {
		pcg_sound_wk.ch[ch].access_step ^= 1;
	}
}

static void pcg_sound_out_counter0(byte data)
{
	pcg_sound_out_counter_sub(0, data);
	xmame_dev_pcg_out_counter0(data);
}

static void pcg_sound_out_counter1(byte data)
{
	pcg_sound_out_counter_sub(1, data);
	xmame_dev_pcg_out_counter1(data);
}

static void pcg_sound_out_counter2(byte data)
{
	pcg_sound_out_counter_sub(2, data);
	xmame_dev_pcg_out_counter2(data);
}

static void pcg_sound_out_control(byte data)
{
	int ch = (data >> 6) & 0x03;

	if (ch <= 2) {
		byte access = (data >> 4) & 0x03;

		if (access >= 1) {
			pcg_sound_wk.ch[ch].control = data;
			pcg_sound_wk.ch[ch].access = access;
			pcg_sound_wk.ch[ch].access_step = 0;
		}
	}
	xmame_dev_pcg_out_control(data);
}

static void pcg_output_after_stateload(void)
{
	int ch;

	xmame_dev_pcg_out_gate(pcg_sound_wk.gate);

	for (ch = 0; ch < 3; ch++) {
		int i;
		byte data;

		data = (ch << 6) | (pcg_sound_wk.ch[ch].access << 4) | 0;
		xmame_dev_pcg_out_control(data);

		for (i = 0; i < 2; i++) {
			data = pcg_sound_wk.ch[ch].counter[ i ];
		...
		data = pcg_sound_wk.ch[ch].control;
		xmame_dev_pcg_out_control(data);
	}
}
...
/* メインメモリ リードポインタ  */
static byte *read_mem_0000_5fff;
...
/* メインメモリ ライトポインタ  */
static byte *write_mem_0000_7fff;
...
INLINE void main_memory_vram_mapping(void)
{
	if (misc_ctrl & MISC_CTRL_EVRAM) {
		...
		if (ALU2_ctrl & ALU2_CTRL_VACCESS) {
			/* VRAM拡張アクセス */
		...
		/* 独立アクセスモード */
		if (memory_bank == MEMORY_BANK_MAIN) {
			/* MAIN RAMアクセス */
			vram_access_way = VRAM_NOT_ACCESS;
		} else {
			/* VRAMアクセス     */
...
byte main_fetch(word addr)
{
	...
	if (memory_wait) {
		...
		if (high_mode == FALSE) {
			...
			/* M1サイクルウェイト */
			z80main_cpu.state0 += 1;

			if (dma_wait_count) {
				/* DMAウェイトがあればすこしずつ加算していく */
				dma_wait_count --;
			...
			if (addr >= 0xf000 && mem_wait_highram) {
				/* 高速RAMのフェッチは M1サイクルウェイト */
				z80main_cpu.state0 += 1;
			}
		}
		...
		if (!(highspeed_flag) && highspeed_n88rom) {
			...
			for (i = 0; highspeed_routine[i] != EndofBasicAddr; i++) {
				if (addr == highspeed_routine[i]) {
					highspeed_flag = TRUE;
					ret_addr =
						main_mem_read(z80main_cpu.SP.W) +
	...
	if (addr < 0x6000) {
		...
		return  read_mem_6000_7fff[ addr & 0x1fff ];
	} else if (addr < 0x8400) {
		if (read_mem_8000_83ff) {
			return  read_mem_8000_83ff[ addr & 0x03ff ];
		} else {
			addr = (addr & 0x03ff) + window_offset;
			if (addr < 0xf000) {
				return  main_ram[ addr ];
			} else {
				return  main_high_ram[ addr & 0x0fff ];
			}
		}
	} else if (addr < 0xc000) {
		return  main_ram[ addr ];
		...
		case VRAM_ACCESS_ALU:
			return  ALU_read(addr & 0x3fff);
		case VRAM_ACCESS_BANK:
			return  vram_read(addr & 0x3fff);
		default:
			if (addr < 0xf000) {
				return  read_mem_c000_efff[ addr & 0x3fff ];
			} else {
				return  read_mem_f000_ffff[ addr & 0x0fff ];
			}
		}
	}
}
...
void main_mem_write(word addr, byte data)
{
	if (addr < 0x8000) {
		...
		if (write_mem_8000_83ff) {
			write_mem_8000_83ff[ addr & 0x03ff ] = data;
			...
			if (addr < 0xf000) {
				...
				main_high_ram[ addr & 0x0fff ] = data;
			}
		}
		...
		switch (vram_access_way) {
		case VRAM_ACCESS_ALU:
			ALU_write(addr & 0x3fff, data);
			break;
		case VRAM_ACCESS_BANK:
			vram_write(addr & 0x3fff, data);
			break;
		default:
			if (addr < 0xf000) {
				write_mem_c000_efff[ addr & 0x3fff ] = data;
			} else {
				write_mem_f000_ffff[ addr & 0x0fff ] = data;
			}
		}
	}
}
...
void main_io_out(byte port, byte data)
{
	...
	case 0x02:
		...
		/* if (use_pcg) */
		pcg_sound_out_gate(data);
		return;

	case 0x0c:
		/* if (use_pcg) */
		pcg_sound_out_counter0(data);
		return;
	case 0x0d:
		/* if (use_pcg) */
		pcg_sound_out_counter1(data);
		return;
	case 0x0e:
		/* if (use_pcg) */
		pcg_sound_out_counter2(data);
		return;
	case 0x0f:
		/* if (use_pcg) */
		pcg_sound_out_control(data);
		return;
	...
	case 0x21:
		...
		/* システムコントロール出力 */
	case 0x30:
		/* SYS_CTRL_MONO は無視 (テキストのカラーはCRTC設定にて決定) */
		if ((sys_ctrl ^ data) & (SYS_CTRL_80)) {
			set_screen_dirty_all();
		}

		if (sio_tape_readable()) {
			if ((sys_ctrl & 0x08) && !(data & 0x08)) {
				sio_check_cmt_error();
			}
		}

		/* カセットモーターリレー音 */
		if ((~sys_ctrl &  data) & 0x08) {
			xmame_dev_sample_motoron();
		}
		if ((sys_ctrl & ~data) & 0x08) {
			xmame_dev_sample_motoroff();
		}
	...
	case 0xff:
		if (data & 0x80) {
			pio_set_mode(PIO_SIDE_M, data);
		} else {
			pio_write_C(PIO_SIDE_M, data);
		}
		return;
	...
	case 0xb5:
		...
		/* ??? Access in N88-BASIC ver 1.8 */
	case 0xc1:
	case 0xf3:
	case 0xf4:
	case 0xf8:

		/* ??? Access in N-BASIC ver 1.8 */
	case 0xe7:
...
byte main_io_in(byte port)
{
	...
	case 0x46:
		if (sound_port & SD_PORT_46_47) {
			return sound2_in_status();
		} else {
			return 0xff;
		}
	case 0x47:
		if (sound_port & SD_PORT_46_47) {
			return sound2_in_data();
		} else {
			return 0xff;
		}
	...
	case 0x6e:
		if (ROM_VERSION >= '8') {
			return cpu_clock | 0x10; /* FH/MH 以降に対応 */
		} else {
			return 0xff;
		}
	...
	case 0x6f:
		if (ROM_VERSION >= '8') {
			return baudrate_sw | 0xf0; /* FH/MH 以降に対応 */
		} else {
			return 0xff;
		}
...
int sio_tape_rewind(void)
{
	...
	if (fp_ti) {
		if (osd_fseek(fp_ti, 0, SEEK_END)) {
			goto ERR;
		}
		if ((cmt_size = osd_ftell(fp_ti)) < 0) {
			goto ERR;
		}
		...
		size = (int)osd_fread(buf, sizeof(char), sizeof(buf), fp_ti);
		if (size == sizeof(buf) &&
			memcmp(buf, T88_HEADER_STR, sizeof(buf)) == 0) {
			/* T88 */
			cmt_is_t88     = TRUE;
			cmt_block_size = 0;
			cmt_EOF        = FALSE;
...
static int sio_getc(int is_cmt, int *tick)
{
	...
	if (is_cmt == FALSE) {
		...
		if (c == EOF) {
			...
			statusbar_message(STATUS_WARN, "Serial input  [EOF]", 0);
			com_EOF = TRUE;
		}
		return c;
		...
		if (c == EOF) {
			cmt_EOF = TRUE;
			statusbar_message(STATUS_WARN, "Tape Read  [EOF]", 0);
		} else {
			cmt_read_chars ++;
		}
...
static void sio_out_command(byte data)
{
	if (sio_instruction == 0) {
		...
		if ((sio_command & 0x04) == 0) {
			/* リセットor受信禁止なら、受信ワークをクリアする */
			sio_data_exist = FALSE;
			RS232C_flag   = FALSE;
		}
...
static byte sio_in_status(void)
{
	...
	if (sio_command & 0x04) {
		...
		if ((sys_ctrl & 0x20) == 0 &&
			sio_tape_readable() &&
			cmt_intr == FALSE) {
			/* テープで、SIO割り込みを使わない場合、ここで読む */

			cmt_dummy_read_cnt ++;
			if (cmt_dummy_read_cnt >= 2) {
				/* IN 21 を 2回実行する度にテープから1文字読む */
				cmt_dummy_read_cnt = 0;
...
#define BCD2INT(b)		((((b) >> 4) & 0x0f ) * 10 + ((b) & 0x0f))
	...
	if (x == 0) {
		...
		if (i >= 38) {
			t.tm_year = 1900 + i - 1900;
		} else {
			t.tm_year = 2000 + i - 1900;
		}
	}
...
static void out_ctrl_signal(byte data)
{
...
static byte in_ctrl_signal(void)
{
	return ((ctrl_vrtc    << 5) |
			(calendar_cdo << 4) |
			ctrl_boot           |
...
void power_on_ram_init(void)
{
	...
	for (addr = 0; addr < 0x10000; addr += 0x100) {
		if ((addr & 0x0d00) == 0x0100 || (addr & 0x0f00) == 0x0500 ||
			(addr & 0x0f00) == 0x0a00 || (addr & 0x0d00) == 0x0c00) {
			data = 0xff;
		} else {
			data = 0x00;
		}
	...
	for (addr = 0xf000; addr < 0x10000; addr++) {
		main_ram[addr] ^= 0xff;
	}
}
...
static T_SUSPEND_W suspend_pc88main_work3[] = {
	{ TYPE_CHAR,  &pcg_sound_wk.gate,				},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[0].control,		},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[0].access,		},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[0].counter[0],	},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[0].counter[1],	},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[0].access_step,	},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[1].control,		},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[1].access,		},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[1].counter[0],	},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[1].counter[1],	},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[1].access_step,	},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[2].control,		},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[2].access,		},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[2].counter[0],	},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[2].counter[1],	},
	{ TYPE_CHAR,  &pcg_sound_wk.ch[2].access_step,	},
	{ TYPE_END,   0                       			},
};
...
int statesave_pc88main(void)
{
	...
	if (statesave_table(SID3, suspend_pc88main_work3) != STATE_OK) {
		return FALSE;
	}
...
int stateload_pc88main(void)
{
	...
	if (stateload_table(SID3, suspend_pc88main_work3) != STATE_OK) {

		/* 旧バージョンなら、みのがす */

		printf("stateload : Statefile is old. (ver ~0.7.2?)\n");
	}
