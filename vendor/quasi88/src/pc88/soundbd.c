...
void sound_out_data(byte data)
{
	...
	case 0x25:
		sound_TIMER_A =
			((int)sound_reg[ 0x24 ] << 2) | (sound_reg[ 0x25 ] & 0x03);
...
byte sound_in_data(int always_sound_II)
{
	...
	if (sound_reg_select < 0x10) {
		/* 0x00..0x0f はリード可 */

		return sound_reg[ sound_reg_select ];

	} else if (sound_reg_select == 0xff) {
		/* 0xff は、SD/SD II の判定 */
...
void sound2_out_data(byte data)
{
	 ...
	 * 		   (int)sound2_data_addr, (int)sound2_data_addr >> sound2_mem_size,
	 * 		   (int)sound2_data_addr & ((1 << sound2_mem_size) -1), (int)data);
	 * } else {
...
#undef L
	...
	case 0x03:
		/* スタートアドレス */
		sound2_start_addr  =
			((int)sound2_reg[ 0x03 ] << 8) | sound2_reg[ 0x02 ];
...
int stateload_sound(void)
{
	...
	if (stateload_table(SID2, suspend_sound_work2) != STATE_OK) {
		...
		if (sound_prescaler == 6) {
			sound_prescaler_sel = 2;
		} else if (sound_prescaler == 3) {
			sound_prescaler_sel = 3;
		} else { /* 2 or 1 */
			sound_prescaler_sel = 0;
		}
	}
...
void sound_output_after_stateload(void)
{
	...
	if (xmame_has_sound()) {
...
#if 0
		{
			...
			int gvol = xmame_cfg_get_mixer_volume(XMAME_MIXER_PCG);
			...
			xmame_cfg_set_mixer_volume(XMAME_MIXER_PCG, 0);
		}
#endif
		...
		case 0:
			xmame_dev_sound_out_reg(0x2f);
			xmame_dev_sound_out_data(0);
			break;
		case 1:
			xmame_dev_sound_out_reg(0x2f);
			xmame_dev_sound_out_data(0);
			xmame_dev_sound_out_reg(0x2e);
			xmame_dev_sound_out_data(0);
			break;
		case 3:
			xmame_dev_sound_out_reg(0x2e);
		...
		if (sound_board == SOUND_I) {
			addr = addr_2203;
			size = COUNTOF(addr_2203);
		} else                      {
			addr = addr_2608;
			size = COUNTOF(addr_2608);
		}
		...
		/* 音量復帰 */
		{
			...
			xmame_cfg_set_mixer_volume(XMAME_MIXER_PCG,  gvol);
		}
#endif
