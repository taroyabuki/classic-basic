...
/* IN(00h)..(0Eh) キースキャン
 * key_scan[0]..[14] が、 I/Oポート 00H..0EH と等価。
 * key_scan[15] は、ジョイスティックの一時ワークに使用。
 * key_scan[16]..[17] は、シフトキーの一時ワークに使用。*/
unsigned char key_scan[ 0x12 ];
...
/*
 * mouse_dx, jop1_dx の関係
 *       マウスが移動した時
 *               mouse_dx にその移動量をセットする。
...
{
	FN_FUNC,	/* [0] はダミー */
	FN_FUNC,	FN_FUNC,	FN_FUNC,	FN_FUNC,	FN_FUNC,	/* 未使用   */
...
#define KEYINJECT_QUEUE_SIZE	(64)
...
static T_KEYINJECT_ENTRY keyinject_queue[ KEYINJECT_QUEUE_SIZE ];
static int keyinject_head = 0;
static int keyinject_tail = 0;
static int keyinject_count = 0;

static int is_injection_mode_legal(void)
{
	return (quasi88_is_exec() || quasi88_is_monitor() || quasi88_is_pause());
}

static int ascii_to_key88(int c)
{
	if (c >= 32 && c <= 126) {
		return c;
	}
...
static int count_sequence_entries(const char *sequence)
{
	...
	while (*p != '\0') {
		if (*p == '<') {
			...
			if (ascii_to_key88((unsigned char)*p) == KEY88_INVALID) {
				return -1;
			}
			count += 2;
			p++;
...
static void enqueue_key_event(int key_code, int press_flag)
{
	keyinject_queue[keyinject_tail].key_code = key_code;
	keyinject_queue[keyinject_tail].press_flag = press_flag;
	keyinject_tail = (keyinject_tail + 1) % KEYINJECT_QUEUE_SIZE;
	keyinject_count++;
}

static int enqueue_validated_sequence(const char *sequence)
{
	const char *p = sequence;
	int key_code;
	...
	while (*p != '\0') {
		if (*p == '<') {
			...
			key_code = ascii_to_key88((unsigned char)*p);
			p++;
		}

		enqueue_key_event(key_code, TRUE);
		total_enqueued++;
		enqueue_key_event(key_code, FALSE);
		total_enqueued++;
	}
...
static int parse_and_enqueue(const char *sequence)
{
	int needed;
	...
	needed = count_sequence_entries(sequence);
	if (needed < 0) {
		return -2;
	}
	if (needed == 0) {
	...
	if (keyinject_count + needed > KEYINJECT_QUEUE_SIZE) {
		return -3;
	}

	enqueue_validated_sequence(sequence);
	return 0;
}

int quasi88_keyinject(const char *sequence)
{
	if (!is_injection_mode_legal()) {
		return -1;
	}

	return parse_and_enqueue(sequence);
}

int quasi88_keyinject_drain(int *code_out, int *press_flag_out)
{
	if (keyinject_count == 0) {
		return FALSE;
	}

	*code_out = keyinject_queue[keyinject_head].key_code;
	*press_flag_out = keyinject_queue[keyinject_head].press_flag;
	keyinject_head = (keyinject_head + 1) % KEYINJECT_QUEUE_SIZE;
	keyinject_count--;
	return TRUE;
}

void quasi88_keyinject_reset(void)
{
	keyinject_head = 0;
	keyinject_tail = 0;
	keyinject_count = 0;
...
static void mouse_to_key(int code, int on);
...
static const T_KEYPORT keyport[ KEY88_END ] = {
	{ 0, 0 },                     /*        KEY88_INVALID         = 0,    */
	...
	{     0,    0 },              /*        KEY88_SYS_PREV        = 170,  */
	{     0,    0 },              /*        KEY88_SYS_NEXT        = 171,  */
	{     0,    0 },              /*        KEY88_SYS_FORWARD     = 172,  */
	{     0,    0 },              /*        KEY88_SYS_BACK        = 173,  */
	{     0,    0 },              /*        KEY88_SYS_MODE        = 174,  */
	{ PortC, Bit0 },              /*        KEY88_SYS_F6          = 175,  */
	{ PortC, Bit1 },              /*        KEY88_SYS_F7          = 176,  */
	{ PortC, Bit2 },              /*        KEY88_SYS_F8          = 177,  */
	{ PortC, Bit3 },              /*        KEY88_SYS_F9          = 178,  */
	{ PortC, Bit4 },              /*        KEY88_SYS_F10         = 179,  */

	{ PortC, Bit0 },              /*        KEY88_F6              = 180,  */
	{ PortC, Bit1 },              /*        KEY88_F7              = 181,  */
	{ PortC, Bit2 },              /*        KEY88_F8              = 182,  */
	{ PortC, Bit3 },              /*        KEY88_F9              = 183,  */
	{ PortC, Bit4 },              /*        KEY88_F10             = 184,  */
	{ PortC, Bit5 },              /*        KEY88_BS              = 185,  */
	{ PortC, Bit6 },              /*        KEY88_INS             = 186,  */
	{ PortC, Bit7 },              /*        KEY88_DEL             = 187,  */
	{ PortD, Bit0 },              /*        KEY88_HENKAN          = 188,  */
	{ PortD, Bit1 },              /*        KEY88_KETTEI          = 189,  */
	{ PortD, Bit2 },              /*        KEY88_PC              = 190,  */
	{ PortD, Bit3 },              /*        KEY88_ZENKAKU         = 191,  */
	{ PortE, Bit0 },              /*        KEY88_RETURNL         = 192,  */
	{ PortE, Bit1 },              /*        KEY88_RETURNR         = 193,  */
	{ PortE, Bit2 },              /*        KEY88_SHIFTL          = 194,  */
	{ PortE, Bit3 },              /*        KEY88_SHIFTR          = 195,  */
	...
	{ Port8, Bit1 },              /*        KEY88_SYS_UP          = 248,  */
	{ PortA, Bit1 },              /*        KEY88_SYS_DOWN        = 249,  */
	{ PortA, Bit2 },              /*        KEY88_SYS_LEFT        = 250,  */
	{ Port8, Bit2 },              /*        KEY88_SYS_RIGHT       = 251,  */
	{     0,    0 },              /*        KEY88_SYS_ROMAJI      = 252,  */
	{     0,    0 },              /*        KEY88_SYS_NUM         = 253,  */
	{     0,    0 },              /*        KEY88_SYS_STATUS      = 254,  */
	{     0,    0 },              /*        KEY88_SYS_MENU        = 255,  */
};

static const T_KEYPORT keyport_ext[][2] = {
	{ { Port9, Bit1 }, { PortY, Bit1 }, },  /*  KEY88_SYS_F6          = 175,  */
	{ { Port9, Bit2 }, { PortY, Bit2 }, },  /*  KEY88_SYS_F7          = 176,  */
	{ { Port9, Bit3 }, { PortY, Bit3 }, },  /*  KEY88_SYS_F8          = 177,  */
	{ { Port9, Bit4 }, { PortY, Bit4 }, },  /*  KEY88_SYS_F9          = 178,  */
	{ { Port9, Bit5 }, { PortY, Bit5 }, },  /*  KEY88_SYS_F10         = 179,  */
	{ { Port9, Bit1 }, { PortY, Bit6 }, },  /*  KEY88_F6              = 180,  */
	{ { Port9, Bit2 }, { PortY, Bit7 }, },  /*  KEY88_F7              = 181,  */
	{ { Port9, Bit3 }, { PortZ, Bit0 }, },  /*  KEY88_F8              = 182,  */
	{ { Port9, Bit4 }, { PortZ, Bit1 }, },  /*  KEY88_F9              = 183,  */
	{ { Port9, Bit5 }, { PortZ, Bit2 }, },  /*  KEY88_F10             = 184,  */
	{ { Port8, Bit3 }, {     0,    0 }, },  /*  KEY88_BS              = 185,  */
	{ { Port8, Bit3 }, { PortZ, Bit3 }, },  /*  KEY88_INS             = 186,  */
	{ { Port8, Bit3 }, {     0,    0 }, },  /*  KEY88_DEL             = 187,  */
	{ { Port9, Bit6 }, {     0,    0 }, },  /*  KEY88_HENKAN          = 188,  */
	{ { Port9, Bit6 }, {     0,    0 }, },  /*  KEY88_KETTEI          = 189,  */
	{ {     0,    0 }, {     0,    0 }, },  /*  KEY88_PC              = 190,  */
	{ {     0,    0 }, {     0,    0 }, },  /*  KEY88_ZENKAKU         = 191,  */
	{ { Port1, Bit7 }, {     0,    0 }, },  /*  KEY88_RETURNL         = 192,  */
	{ { Port1, Bit7 }, {     0,    0 }, },  /*  KEY88_RETURNR         = 193,  */
	{ {     0,    0 }, { PortZ, Bit4 }, },  /*  KEY88_SHIFTL          = 194,  */
	{ {     0,    0 }, { PortZ, Bit5 }, },  /*  KEY88_SHIFTR          = 195,  */
};
...
#define IS_KEY88_LATERTYPE(c)	(KEY88_SYS_F6 <= (c) && (c) <= KEY88_SHIFTR)

#define IS_KEY88_SYSTEMKEY(c)	((KEY88_SYS_PREV <= (c) && (c) <= KEY88_SYS_F10) ||	\
								 (KEY88_SYS_UP <= (c) && (c) <= KEY88_SYS_MENU))


#define KEY88_PRESS(code)												\
	do {																\
		if (IS_KEY88_LATERTYPE((code))) {								\
			int lcode = (code) - KEY88_SYS_F6;							\
			key_scan[ keyport_ext[lcode][0].port ] &= ~keyport_ext[lcode][0].mask; \
			key_scan[ keyport_ext[lcode][1].port ] &= ~keyport_ext[lcode][1].mask; \
		}																\
		key_scan[ keyport[(code)].port ] &= ~keyport[(code)].mask;		\
		if ((key_scan[ PortY ] & key_scan[ PortZ ]) != 0xff) {			\
			key_scan[ Port8 ] &= ~Bit6;									\
		}																\
	} while (0)
...
static void clr_key_function(void)
{
	...
	/* システムのキーは、機能固定 */
	key_func[ KEY88_SYS_PREV   ] = FN_PREV_KEY;
	key_func[ KEY88_SYS_NEXT   ] = FN_NEXT_KEY;
	key_func[ KEY88_SYS_F6     ] = KEY88_F6;
	key_func[ KEY88_SYS_F7     ] = KEY88_F7;
	key_func[ KEY88_SYS_F8     ] = KEY88_F8;    
	key_func[ KEY88_SYS_F9     ] = KEY88_F9;
	key_func[ KEY88_SYS_F10    ] = KEY88_F10;
	key_func[ KEY88_SYS_UP     ] = KEY88_UP;
	key_func[ KEY88_SYS_DOWN   ] = KEY88_DOWN;
	key_func[ KEY88_SYS_RIGHT  ] = KEY88_RIGHT;
	key_func[ KEY88_SYS_LEFT   ] = KEY88_LEFT;
	key_func[ KEY88_SYS_ROMAJI ] = FN_ROMAJI_KEY;
	key_func[ KEY88_SYS_NUM    ] = FN_NUM_KEY;
...
#define ASSIGN_KEY_MODE_NONE		(0)
#define ASSIGN_KEY_MODE_SYS			(1)
#define ASSIGN_KEY_MODE_USER		(2)
static void assign_key_by_list(const T_ASSIGN_KEY_TABLE table[],
							   int nr_array,
							   int mode)
{
	int i;
	for (i = 0; i < nr_array; i++) {
		switch (mode) {
		case ASSIGN_KEY_MODE_USER:
			set_key_function(table[i].code, table[i].user);
			break;
		case ASSIGN_KEY_MODE_SYS:
			set_key_function(table[i].code, table[i].func);
			break;
		default:
			set_key_function(table[i].code, 0);
			break;
		}
	}
}
...
/* Num lock を有効に (or 数字キーをテンキーに) */
static void assign_key_numlock_tenkey(void)
{
	static const T_ASSIGN_KEY_TABLE table1[] = {
		{ KEY88_1,          KEY88_F1,          },
		{ KEY88_2,          KEY88_F2,          },
		{ KEY88_3,          KEY88_F3,          },
		{ KEY88_4,          KEY88_F4,          },
		...
		{ KEY88_8,          KEY88_KP_8,        },
		{ KEY88_9,          KEY88_KP_9,        },
		{ KEY88_0,          KEY88_KP_MULTIPLY, },
		{ KEY88_MINUS,      KEY88_KP_SUB,      },
		{ KEY88_e,          KEY88_UP,          },
		{ KEY88_y,          KEY88_HELP,        },
		{ KEY88_u,          KEY88_KP_4,        },
		{ KEY88_i,          KEY88_KP_5,        },
		{ KEY88_o,          KEY88_KP_6,        },
		{ KEY88_p,          KEY88_KP_ADD,      },
		{ KEY88_AT,         KEY88_KP_DIVIDE,   },
		{ KEY88_s,          KEY88_LEFT,        },
		{ KEY88_d,          KEY88_RIGHT,       },
		{ KEY88_j,          KEY88_KP_1,        },
		{ KEY88_k,          KEY88_KP_2,        },
		{ KEY88_l,          KEY88_KP_3,        },
		{ KEY88_SEMICOLON,  KEY88_KP_EQUAL,    },
		{ KEY88_x,          KEY88_DOWN,        },
		{ KEY88_m,          KEY88_KP_0,        },
		{ KEY88_COMMA,      KEY88_KP_COMMA,    },
		{ KEY88_PERIOD,     KEY88_KP_PERIOD,   },
		{ KEY88_SLASH,      KEY88_RETURNR,     },

		{ KEY88_EXCLAM,     KEY88_F1,          },
		{ KEY88_QUOTEDBL,   KEY88_F2,          },
		{ KEY88_NUMBERSIGN, KEY88_F3,          },
		{ KEY88_DOLLAR,     KEY88_F4,          },
		{ KEY88_PERCENT,    KEY88_F5,          },
		{ KEY88_AMPERSAND,  KEY88_HOME,        },
		{ KEY88_APOSTROPHE, KEY88_KP_7,        },
		{ KEY88_PARENLEFT,  KEY88_KP_8,        },
		{ KEY88_PARENRIGHT, KEY88_KP_9,        },
		{ KEY88_0,          KEY88_KP_MULTIPLY, },
		{ KEY88_EQUAL,      KEY88_KP_SUB,      },
		{ KEY88_E,          KEY88_UP,          },
		{ KEY88_Y,          KEY88_HELP,        },
		{ KEY88_U,          KEY88_KP_4,        },
		{ KEY88_I,          KEY88_KP_5,        },
		{ KEY88_O,          KEY88_KP_6,        },
		{ KEY88_P,          KEY88_KP_ADD,      },
		{ KEY88_BACKQUOTE,  KEY88_KP_DIVIDE,   },
		{ KEY88_S,          KEY88_LEFT,        },
		{ KEY88_D,          KEY88_RIGHT,       },
		{ KEY88_J,          KEY88_KP_1,        },
		{ KEY88_K,          KEY88_KP_2,        },
		{ KEY88_L,          KEY88_KP_3,        },
		{ KEY88_PLUS,       KEY88_KP_EQUAL,    },
		{ KEY88_X,          KEY88_DOWN,        },
		{ KEY88_M,          KEY88_KP_0,        },
		{ KEY88_LESS,       KEY88_KP_COMMA,    },
		{ KEY88_GREATER,    KEY88_KP_PERIOD,   },
		{ KEY88_QUESTION,   KEY88_RETURNR,     },
	};
	static const T_ASSIGN_KEY_TABLE table2[] = {
		{ KEY88_1,          KEY88_KP_1,        },
		{ KEY88_2,          KEY88_KP_2,        },
		{ KEY88_3,          KEY88_KP_3,        },
		{ KEY88_4,          KEY88_KP_4,        },
		...
		{ KEY88_EXCLAM,     KEY88_KP_1,        },
		{ KEY88_QUOTEDBL,   KEY88_KP_2,        },
		{ KEY88_NUMBERSIGN, KEY88_KP_3,        },
		{ KEY88_DOLLAR,     KEY88_KP_4,        },
		{ KEY88_PERCENT,    KEY88_KP_5,        },
		{ KEY88_AMPERSAND,  KEY88_KP_6,        },
		{ KEY88_APOSTROPHE, KEY88_KP_7,        },
		{ KEY88_PARENLEFT,  KEY88_KP_8,        },
		{ KEY88_PARENRIGHT, KEY88_KP_9,        },
	};
	...
	if (numlock_emu) {
		/* Num lock を有効に */
		assign_key_by_list(table1, COUNTOF(table1), ASSIGN_KEY_MODE_SYS);

	} else {
		assign_key_by_list(table1, COUNTOF(table1), ASSIGN_KEY_MODE_NONE);

		if (quasi88_is_exec()) {

			if (tenkey_emu) {
				/* 数字キーをテンキーに設定 (EMU モード中のみ) */
				assign_key_by_list(table2, COUNTOF(table2), ASSIGN_KEY_MODE_SYS);
			}
		}
	}
}

static void assign_key_cursorkey(void)
{
	static T_ASSIGN_KEY_TABLE table[] = {
		{ KEY88_UP,		KEY88_KP_8, },
		{ KEY88_DOWN,   KEY88_KP_2, },
		{ KEY88_LEFT,   KEY88_KP_4, },
		{ KEY88_RIGHT,  KEY88_KP_6, },
	};

	if (cursor_key_mode == 2) {
		size_t i;
		for (i = 0; i < COUNTOF(table); i++) {
			table[i].user = cursor_key_assign[i];
		}
	}
	assign_key_by_list(table, COUNTOF(table), cursor_key_mode);
	/* cursor_key_mode == 0 の場合は、割り当てなし(カーソルキーのまま)となる */
}

static void assign_key_mouse_joystick(void)
{
	/*
	...
	*/
	...
	/* マウスの入力割り当て変更 */
	{
		static T_ASSIGN_KEY_TABLE table1[] = {
			{ KEY88_MOUSE_UP,    KEY88_KP_8, },
			{ KEY88_MOUSE_DOWN,  KEY88_KP_2, },
			{ KEY88_MOUSE_LEFT,  KEY88_KP_4, },
			{ KEY88_MOUSE_RIGHT, KEY88_KP_6, },
			{ KEY88_MOUSE_L,     KEY88_x,    },
			{ KEY88_MOUSE_R,     KEY88_z,    },
		};
		const static T_ASSIGN_KEY_TABLE table2[] = {
			{ KEY88_MOUSE_UP,    KEY88_PAD1_UP,    },
			{ KEY88_MOUSE_DOWN,  KEY88_PAD1_DOWN,  },
			{ KEY88_MOUSE_LEFT,  KEY88_PAD1_LEFT,  },
			{ KEY88_MOUSE_RIGHT, KEY88_PAD1_RIGHT, },
			{ KEY88_MOUSE_L,     KEY88_PAD1_A,     },
			{ KEY88_MOUSE_R,     KEY88_PAD1_B,     },
		};
		const static T_ASSIGN_KEY_TABLE table3[] = {
			{ KEY88_MOUSE_L,     KEY88_PAD1_A,     },
			{ KEY88_MOUSE_R,     KEY88_PAD1_B,     },
		};
		int swap = FALSE;
		...
		case MOUSE_NONE:
		case MOUSE_JOYSTICK:
			/* マウスポート未接続なら、マウス移動をテンキーetcに割り当て可 */
			if (mouse_key_mode) {
				if (mouse_key_mode == 2) {
					size_t i;
					for (i = 0; i < COUNTOF(table1); i++) {
						table1[i].user = mouse_key_assign[i];
					}
				}
				assign_key_by_list(table1, COUNTOF(table1), mouse_key_mode);
				if (mouse_key_mode == 1) {
		...
		case MOUSE_MOUSE:
			/* マウスポートに、マウス接続中 */
			assign_key_by_list(table3, COUNTOF(table3), ASSIGN_KEY_MODE_SYS);
			swap = mouse_swap_button;
			break;
		}
		if (swap) {
	...
	/* ジョイスティック入力割り当て変更 */
	{
		static T_ASSIGN_KEY_TABLE table4[] = {
			{ KEY88_PAD1_UP,    KEY88_KP_8, },
			{ KEY88_PAD1_DOWN,  KEY88_KP_2, },
			{ KEY88_PAD1_LEFT,  KEY88_KP_4, },
			{ KEY88_PAD1_RIGHT, KEY88_KP_6, },
			{ KEY88_PAD1_A,     KEY88_x,    },
			{ KEY88_PAD1_B,     KEY88_z,    },
			{ KEY88_PAD1_C,     0,          },
			{ KEY88_PAD1_D,     0,          },
			{ KEY88_PAD1_E,     0,          },
			{ KEY88_PAD1_F,     0,          },
		...
		assign_key_by_list(table4, COUNTOF(table4), ASSIGN_KEY_MODE_NONE);

		switch (mouse_mode) {
		case MOUSE_NONE:
		case MOUSE_MOUSE:
		case MOUSE_JOYMOUSE:
			/* マウスポートに、ジョイスティックは接続されてない */
			if (joy_key_mode) {
				if (joy_key_mode == 2) {
					size_t i;
					for (i = 0; i < COUNTOF(table4); i++) {
						table4[i].user = joy_key_assign[i];
					}
				}
				assign_key_by_list(table4, COUNTOF(table4), joy_key_mode);
				if (joy_key_mode == 1) {
	...
	/* ジョイスティック２入力割り当て変更 */
	{
		static T_ASSIGN_KEY_TABLE table5[] = {
			{ KEY88_PAD2_UP,    KEY88_KP_8, },
			{ KEY88_PAD2_DOWN,  KEY88_KP_2, },
			{ KEY88_PAD2_LEFT,  KEY88_KP_4, },
			{ KEY88_PAD2_RIGHT, KEY88_KP_6, },
			{ KEY88_PAD2_A,     KEY88_x,    },
			{ KEY88_PAD2_B,     KEY88_z,    },
			{ KEY88_PAD2_C,     0,          },
			{ KEY88_PAD2_D,     0,          },
			{ KEY88_PAD2_E,     0,          },
			{ KEY88_PAD2_F,     0,          },
		...
		assign_key_by_list(table5, COUNTOF(table5), ASSIGN_KEY_MODE_NONE);

		if (joy2_key_mode) {
			if (joy2_key_mode == 2) {
				size_t i;
				for (i = 0; i < COUNTOF(table5); i++) {
					table5[i].user = joy2_key_assign[i];
				}
			}
			assign_key_by_list(table5, COUNTOF(table5), joy2_key_mode);
			if (joy2_key_mode == 1) {
...
static void assign_key_functionkey_on(void)
{
...
void keyboard_switch(void)
{
	if (quasi88_is_exec()) {
		...
		/* マウス、ジョイスティックの入力割り当て変更 */
		assign_key_mouse_joystick();

		/* ファンクションキーの機能割り当て */
		assign_key_functionkey_on();
		...
		/* ファンクションキーの機能割り当て (基本的には機能は無効) */
		assign_key_functionkey_on();

		/* 全てのキーを離す (カナ/CAPS/NUMは除く) */
 ...
 * 全てのキーを離す (カナ/CAPS/NUMは除く)
 *****************************************************************************/
void keyboard_all_release(void)
{
	size_t i;
	/* キースキャンワーク初期化 */
	for (i = 0; i < sizeof(key_scan); i++) {
	...
	quasi88_notify_touchkey(TOUCHKEY_NOTIFY_REQ_SHIFT_OFF);
	quasi88_notify_touchkey(TOUCHKEY_NOTIFY_REQ_SHIFTL_OFF);
	quasi88_notify_touchkey(TOUCHKEY_NOTIFY_REQ_SHIFTR_OFF);
...
void keyboard_update(void)
{
	...
	if (quasi88_is_exec()) {
		if (bridge_drain_injected_key_event(&code, &press_flag)) {
			quasi88_key(code, press_flag);
		}
	...
	case MOUSE_JOYMOUSE:
		...
		mouse_to_key(KEY88_MOUSE_UP, (status & PadU));
		mouse_to_key(KEY88_MOUSE_DOWN, (status & PadD));
		mouse_to_key(KEY88_MOUSE_LEFT, (status & PadL));
		mouse_to_key(KEY88_MOUSE_RIGHT, (status & PadR));
...
static void record_playback(void)
{
	...
	if (fp_pb) {
		...
		if (osd_fread(&key_record, sizeof(char), sizeof(key_record),
					  fp_pb) == sizeof(key_record)) {
			for (i = 0; i < 0x10; i++) {
			...
			statusbar_message(STATUS_INFO, "Playback  [EOF]", 0);
			osd_fclose(fp_pb);
			fp_pb = NULL;
		}
	}
...
#if 0
	/* マウス座標も初期化したほうがいいかもしれないけど、
	   そこまでしなくてもいっか */
	event_get_mouse_pos(&mouse_x, &mouse_y);
	screen_mouse_movement_adjust(&mouse_x, &mouse_y);
#endif
	mouse_dx = 0;
	mouse_dy = 0;

#endif
	/* でも、実際にはこれをしなくても実害はでないと思う。*/
...
void keyboard_jop1_strobe(void)
{
	if (mouse_mode == MOUSE_MOUSE       &&
			   ...
			   2回目以降のストローブで、この確定した値を転送する */
		{
			...
			if (f == 0x01) {
				/* x変位が ±127を超えた場合 x変位をmax値にしてyを補正 */
				dy = 127 * SGN(dx) * dy / dx;
				dx = 127 * SGN(dx);
			} else if (f == 0x02) {
				/* y変位が ±127を超えた場合  y変位をmax値にしてxを補正 */
				dx = 127 * SGN(dy) * dx / dy;
				dy = 127 * SGN(dy);
			}
#endif
 ...
 * カナ（ローマ字）キーロック
 ************************************************************************/
int  quasi88_now_key_numlock(void)
{
	return numlock_emu;
}
void quasi88_tgl_key_numlock(void)
{
	numlock_emu ^= 1;
	assign_key_numlock_tenkey();
...
void quasi88_cfg_key_numlock(int on)
{
	if (((numlock_emu)          && (on == FALSE)) ||
		...
		quasi88_tgl_key_numlock();
	}
}

int  quasi88_now_key_caps(void)
{
	return IS_KEY88_PRESS(KEY88_CAPS);
}
void quasi88_tgl_key_caps(void)
{
...
void quasi88_cfg_key_caps(int on)
{
	if (((IS_KEY88_PRESS(KEY88_CAPS))          && (on == FALSE)) ||
		((IS_KEY88_PRESS(KEY88_CAPS) == FALSE) && (on))) {

		quasi88_tgl_key_caps();
	}
}

int  quasi88_now_key_kana(void)
{
	return IS_KEY88_PRESS(KEY88_KANA);
}
void quasi88_tgl_key_kana(void)
{
	...
	quasi88_notify_touchkey(TOUCHKEY_NOTIFY_REQ_ROMAJI_OFF);
}
void quasi88_cfg_key_kana(int on)
{
	if (((IS_KEY88_PRESS(KEY88_KANA))          && (on == FALSE)) ||
		...
		quasi88_tgl_key_kana();
	}
}

int  quasi88_now_key_romaji(void)
{
	return romaji_input_mode;
}
void quasi88_tgl_key_romaji(void)
{
	KEY88_TOGGLE(KEY88_KANA);
	romaji_input_mode = FALSE;
	...
	quasi88_notify_touchkey(IS_KEY88_PRESS(KEY88_KANA) ?
							TOUCHKEY_NOTIFY_REQ_KANA_ON :
							TOUCHKEY_NOTIFY_REQ_KANA_OFF);
	quasi88_notify_touchkey(romaji_input_mode ? 
...
static void quasi88_set_last_key_function(int code);

void quasi88_key(int code, int on)
{
	int notify_code[2];

	/* SHIFT + CTRL 押下で、マウスグラブ解除 */
	{
		static char shift_on = FALSE;
		static char ctrl_on = FALSE;
		if (code == KEY88_CTRL ||
			code == KEY88_SHIFT ||
			code == KEY88_SHIFTL ||
			code == KEY88_SHIFTR) {
	...
	if (quasi88_is_exec()) {
		...
		unsigned char p08 = key_scan[ Port8 ];

		if (key_func[ code ]) {
			/* 特殊処理割り当て済の場合 特殊機能処理を実行 */
			code = do_func(key_func[ code ], on);
			if (code == 0) {
				/* 戻値は 新キーコード */
				return;
			}
		...
		if ((p08 ^ key_scan[ Port8 ]) & Bit6) {
			notify_code[1] = KEY88_SHIFT;
		} else {
			notify_code[1] = 0;
		}

	} else if (quasi88_is_menu_mode()) {
		int i;
		static const T_ASSIGN_KEY_TABLE menukey[] ={
			{ KEY88_KP_0,			KEY88_0,		},
			{ KEY88_KP_1,			KEY88_1,		},
			{ KEY88_KP_2,			KEY88_2,		},
			{ KEY88_KP_3,			KEY88_3,		},
			...
			{ KEY88_KP_8,			KEY88_8,		},
			{ KEY88_KP_9,			KEY88_9,		},
			{ KEY88_KP_MULTIPLY,	KEY88_ASTERISK,	},
			{ KEY88_KP_ADD,			KEY88_PLUS,		},
			{ KEY88_KP_EQUAL,		KEY88_EQUAL,	},
			{ KEY88_KP_COMMA,		KEY88_COMMA,	},
			{ KEY88_KP_PERIOD,		KEY88_PERIOD,	},
			{ KEY88_KP_SUB,			KEY88_MINUS,	},
			{ KEY88_KP_DIVIDE,		KEY88_SLASH,	},
			{ KEY88_INS_DEL,		KEY88_BS,		},
/*			{ KEY88_DEL,			KEY88_BS,		},*/ /* DELのまま */
			{ KEY88_KETTEI,			KEY88_SPACE,	},
			{ KEY88_HENKAN,			KEY88_SPACE,	},
			{ KEY88_RETURNL,		KEY88_RETURN,	},
			{ KEY88_RETURNR,		KEY88_RETURN,	},
			{ KEY88_SHIFTL,			KEY88_SHIFT,	},
			{ KEY88_SHIFTR,			KEY88_SHIFT,	},
		};

		/* メニューではファンクションキーの機能割り当ては基本的に無効だが
		   あとから情報を引っ張ってこれるように、一時的に覚えておく */
		quasi88_set_last_key_function(code);

		/* メニューでは特殊処理割り当ては無効だが、以下だけは処理する */
		switch (code) {
		case KEY88_SYS_PREV:
			quasi88_notify_touchkey(TOUCHKEY_NOTIFY_REQ_PREV);
			return;
		case KEY88_SYS_NEXT:
			quasi88_notify_touchkey(TOUCHKEY_NOTIFY_REQ_NEXT);
			return;
		case KEY88_SYS_NUM:
			quasi88_cfg_key_numlock(on);
			return;
		}

		/* 先に、 NUM lock 時のキーや、システムのキーを読み替え */
		if (key_func[ code ] >= FN_end) {
			code = key_func[ code ];
		}

		/* 次に、メニュー用にキーを読み替え */
		for (i = 0; i < COUNTOF(menukey); i++) {
			if (code == menukey[i].code) {
				code = menukey[i].func;
				break;
			}
		}
	...
	/* シフト、カナ、CAPSキーは、タッチキーにも状態を通知する */
	{
		static const struct {
			unsigned char code;
			unsigned char on_val;
			unsigned char off_val;
		} table[] = {
			{ KEY88_SHIFT,  TOUCHKEY_NOTIFY_REQ_SHIFT_ON,  TOUCHKEY_NOTIFY_REQ_SHIFT_OFF,  },
			{ KEY88_SHIFTL, TOUCHKEY_NOTIFY_REQ_SHIFTL_ON, TOUCHKEY_NOTIFY_REQ_SHIFTL_OFF, },
			{ KEY88_SHIFTR, TOUCHKEY_NOTIFY_REQ_SHIFTR_ON, TOUCHKEY_NOTIFY_REQ_SHIFTR_OFF, },
			{ KEY88_KANA,   TOUCHKEY_NOTIFY_REQ_KANA_ON,   TOUCHKEY_NOTIFY_REQ_KANA_OFF,   },
			{ KEY88_CAPS,   TOUCHKEY_NOTIFY_REQ_CAPS_ON,   TOUCHKEY_NOTIFY_REQ_CAPS_OFF,   },
			{ KEY88_CTRL,   TOUCHKEY_NOTIFY_REQ_CTRL_ON,   TOUCHKEY_NOTIFY_REQ_CTRL_OFF,   },
			{ KEY88_GRAPH,  TOUCHKEY_NOTIFY_REQ_GRAPH_ON,  TOUCHKEY_NOTIFY_REQ_GRAPH_OFF,  },
		};
		size_t i;
		for (i = 0; i < COUNTOF(table); i++) {
			if (notify_code[0] == table[i].code ||
				notify_code[1] == table[i].code) {
				if (on) {
					quasi88_notify_touchkey( table[i].on_val );
				} else {
					quasi88_notify_touchkey( table[i].off_val );
				}
			}
		}
	}
#if 0
	{
		...
		if (first == FALSE) {
			printf("    ssrr       ss dib09876       du c/-chldt es54321s cskgiruh _/.,;:98 76543210 -^]\\[ZYX WVUTSRQP ONMLKJIH GFEDCBA@ r.,=+*98 76543210\n");
			first = TRUE;
		}
		for (int i = 0x0e; i >= 0; i--)
			printf("%x%x%x%x%x%x%x%x ",
				   (key_scan[i] >> 7) & 1, (key_scan[i] >> 6) & 1,
				   (key_scan[i] >> 5) & 1, (key_scan[i] >> 4) & 1,
				   (key_scan[i] >> 3) & 1, (key_scan[i] >> 2) & 1,
				   (key_scan[i] >> 1) & 1, (key_scan[i] >> 0) & 1);
		printf("\r");
	}
#endif
}
...
/***********************************************************************
 * メニューモード時の、キーの機能割当を取得
 *
 ************************************************************************/
static int last_key_code;
static int last_key_function;
static void quasi88_set_last_key_function(int code)
{
	last_key_code = code;
	if (key_func[ code ]) {
		last_key_function = key_func[ code ];
	} else {
		last_key_function = code;
	}
}
int quasi88_get_last_key_function(int code)
{
	if (code == last_key_code) {
		return last_key_function;
	} else {
		return FN_FUNC;
	}
}
...
static int do_func(int func, int on)
{
	...
	case FN_STATUS:
		...
		if (on) {
#if 1
			int bit = ((quasi88_cfg_now_toolbar()   ? 0x0 : 0x2) |
					   (quasi88_cfg_now_statusbar() ? 0x0 : 0x1));
			bit = (bit + 1) & 0x3;
			quasi88_cfg_set_toolbar((bit & 0x2) ? FALSE : TRUE);
			quasi88_cfg_set_statusbar((bit & 0x1) ? FALSE : TRUE);
#else
			static const t[] = { 1, 1, 1, 1, 0, 1, 0, 0, 0, 0, 0, 1, 0, };
			static const s[] = { 1, 0, 1, 0, 0, 0, 0, 1, 0, 1, 0, 1, 0, };
			static int cycle = 0;
			cycle = (cycle + 1) % 13;
			quasi88_cfg_set_toolbar(t[cycle]);
			quasi88_cfg_set_statusbar(s[cycle]);
#endif
		}
		return 0;
	case FN_MENU:
 ...
 * マウス移動
 *		dx, dy はマウスの移動量
 *		abs_mode が真なら、マウス移動は絶対座標モード
 *****************************************************************************/
void keyboard_mouse_move(int dx, int dy, int abs_mode)
{
	if (quasi88_is_exec()) {
		if (abs_mode) {
			/* マウスの速度調整はしない */
			...
			/* マウスの速度調整 */
			dx = dx * mouse_sensitivity / 100;
			dy = dy * mouse_sensitivity / 100;
		}
 ...
 *      on   は、キー押下なら真、キー開放なら偽
 *****************************************************************************/
void keyboard_mouse_button(int code, int on)
{
	if (quasi88_is_exec()) {

		/* シリアルマウス用のワークセット (キーコード読み替え前) */
 ...
 *
 *----------------------------------------------------------------------*/
int quasi88_now_mouse_mode(void)
{
	return mouse_mode;
}

void quasi88_set_mouse_mode(int mode)
{
	mouse_mode = mode;
	assign_key_mouse_joystick();
...
void quasi88_set_cursor_key_mode(int mode)
{
...
static const T_SYMBOL_TABLE key88sym_list[] = {
	...
	{ "KEY88_F4",               KEY88_F4                },
	{ "KEY88_F5",               KEY88_F5                },
	{ "KEY88_SYS_PREV",         KEY88_SYS_PREV          },
	{ "KEY88_SYS_NEXT",         KEY88_SYS_NEXT          },
	{ "KEY88_SYS_FORWARD",      KEY88_SYS_FORWARD       },
	{ "KEY88_SYS_BACK",         KEY88_SYS_BACK          },
	{ "KEY88_SYS_MODE",         KEY88_SYS_MODE          },
	{ "KEY88_SYS_F6",           KEY88_SYS_F6            },
	{ "KEY88_SYS_F7",           KEY88_SYS_F7            },
	{ "KEY88_SYS_F8",           KEY88_SYS_F8            },
	{ "KEY88_SYS_F9",           KEY88_SYS_F9            },
	{ "KEY88_SYS_F10",          KEY88_SYS_F10           },
	{ "KEY88_F6",               KEY88_F6                },
	{ "KEY88_F7",               KEY88_F7                },
	{ "KEY88_F8",               KEY88_F8                },
	{ "KEY88_F9",               KEY88_F9                },
	...
	{ "KEY88_SYS_UP",           KEY88_SYS_UP            },
	{ "KEY88_SYS_DOWN",         KEY88_SYS_DOWN          },
	{ "KEY88_SYS_LEFT",         KEY88_SYS_LEFT          },
	{ "KEY88_SYS_RIGHT",        KEY88_SYS_RIGHT         },
	{ "KEY88_SYS_ROMAJI",       KEY88_SYS_ROMAJI        },
	{ "KEY88_SYS_NUM",          KEY88_SYS_NUM           },
	{ "KEY88_SYS_STATUS",       KEY88_SYS_STATUS        },
	{ "KEY88_SYS_MENU",         KEY88_SYS_MENU          },

	/* 以下のシンボルは廃止したが、読み替えてサポートする */
	{ "KEY88_F11",              KEY88_SYS_STATUS        },
	{ "KEY88_F12",              KEY88_SYS_MENU          },
};
 ...
 *
 *      setting_callback(int type, int code, int key88, int alter_key88);
 *
 *      上の例の場合、引数は以下がセットされる。
 *              type          … 1
 *              code          … "SDLK_ESCAPE" を int に変換したもの (後述 ※1)
 *              key88         … KEY88_ESC の enum値
 *              alter_key88   … KEY88_KP_4 の enum値
 *
 * 3行目も、キーの「設定行」だが、書式が若干異なっている。
 * この場合も、同様にコールバック関数が呼び出される。
 *
 *      callback(int type, int code, int key88, int alter_key88);
 *
 *      上の例の場合、引数は以下がセットされる。
 *              type          … 2
 *              code          … 49
 *              key88         … KEY88_ZENKAKU の enum値
 *              alter_key88   … -1 (3番目のトークンがないことを意味する)
 *
									...
									int alter_key88))
{
	...
	int  type, code, key88, alter_key88;
	const char *err_mes;
	...
	if (keyconf_filename == NULL) {
		/* ファイル名未指定ならばデフォルト名を取得 */
		filename = filename_alloc_keyboard_cfgname();
	} else {
		...
		if (parm1[0] == '[') {
			...
			if (working) {
				...
				if (parm2 == NULL) {
					...
					if (parm3) {
						alter_key88 = keyboard_str2key88(parm3);
					} else       {
						alter_key88 = -1;
					}
