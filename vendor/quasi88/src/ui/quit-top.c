 ...
 *
 ************************************************************************/
static void cb_quit_cancel(UNUSED_WIDGET, UNUSED_PARM)
{
	quasi88_exec();
}

static void cb_quit(UNUSED_WIDGET, UNUSED_PARM)
{
	quasi88_quit();
}
...
void quit_top(void)
{
	...
	l = q8tk_button_new_with_label("  Quit  ");
	q8tk_widget_show(l);
	q8tk_signal_connect(l, "clicked", cb_quit, NULL);
	q8tk_box_pack_start(Q8TK_DIALOG(d)->action_area, l);

	l = q8tk_button_new_with_label(" Cancel ");
	q8tk_widget_show(l);
	q8tk_signal_connect(l, "clicked", cb_quit_cancel, NULL);
