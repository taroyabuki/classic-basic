...
static void monitor_dump(void)
{
	...
	if (!exist_argv()) {
		...
		/* [<bank>] */
		if (argv_is(ARGV_BANK)) {
			bank = argv.val;
			shift();
		}

		/* <addr> */
		if (!argv_is(ARGV_ADDR)) {
			error();
		}
		start = argv.val;
		shift();
...
static void monitor_dumpext(void)
{
	...
	/* 毎回ダンプしたアドレスを覚えておく (連続ダンプ用) */
	save_dumpext_addr  = start + size;
	save_dumpext_bank  = bank;
		...
		for (j = 0; j < 2; j++) {
			...
			for (i = 0; i < NR_BP; i++) {
				...
				case BP_NONE:
					...
					break;
				case BP_PC:
					printf("PC   reach %04XH\n", addr & 0xffff);
					break;
				case BP_READ:
					printf("READ  from %04XH\n", addr & 0xffff);
					break;
				case BP_WRITE:
					printf("WRITE   to %04XH\n", addr & 0xffff);
					break;
				case BP_IN:
					printf("INPUT from %02XH\n", addr & 0xff);
					break;
				case BP_OUT:
					printf("OUTPUT  to %04XH\n", addr & 0xff);
					break;
				}
			}
		}
...
static void monitor_fbreak(void)
{
	...
	if (exist_argv()) {
		...
		if (action == ARG_READ || action == ARG_WRITE || action == ARG_DIAG) {
			...
			drive = argv.val;
			shift();
			if (! argv_is(ARGV_NUM) || argv.val < 0 || argv.val > 163) {
				error();
			}
			track = argv.val;
			shift();
			if (exist_argv() && ! argv_is(ARGV_SIZE)) {
				if (! argv_is(ARGV_NUM) || argv.val < 0) {
					error();
				}
				sector = argv.val;
				shift();
...
char *command_generator(char *text, int state)
{
	...
	while (count < COUNTOF(monitor_cmd)) {
		/* コマンド名を検索 */
...
void monitor_main(void)
{
	...
	while ((quasi88_event_flags & EVENT_MODE_CHANGED) == 0) {
		...
		case MONITOR_LINE_INPUT:
			...
			printf("QUASI88> ");
			if (fgets(buf, MAX_CHRS, stdin) == NULL) {
				/* ^D が入力されたら */
#ifndef IGNORE_CTRL_D
				/* 強制的に終了。回避方法がわからん */
				quasi88_quit();
#else
				/* IRIX/AIXは大丈夫? */
...
#else
			{
				...
				if (p == NULL) {
					...
					if (*p == '\0' && ph != NULL) {
						/* リターンキーで直前のコマンドを実行 */
						strncpy(buf, ph->line, MAX_CHRS - 1);
					} else {
