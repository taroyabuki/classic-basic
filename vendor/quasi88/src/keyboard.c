...
/*---------------------------------------------------------------------------
 * Queued Input for Headless Automation
 *---------------------------------------------------------------------------*/

/* Queue capacity - enough for typical BASIC input sequences */
#define	KEYINJECT_QUEUE_SIZE	(64)

/* Queue entry: one PC-8801 key code, pending press event */
typedef struct {
    int key_code;		/* KEY88_XXX code */
    int press_flag;		/* TRUE for press, FALSE for release */
} T_KEYINJECT_ENTRY;

static T_KEYINJECT_ENTRY	keyinject_queue[KEYINJECT_QUEUE_SIZE];
static int			keyinject_head = 0;	/* Next dequeue position */
static int			keyinject_tail = 0;	/* Next enqueue position */
static int			keyinject_count = 0;	/* Current queue size */


/*
 * is_injection_mode_legal - Check if current emulator mode allows key injection
 *
 * Legal modes: EXEC, MONITOR, PAUSE
 * Illegal modes: GO, TRACE, STEP, MENU, QUIT
 *
 * Note: GO, TRACE, STEP are sub-modes of EXEC handled internally,
 * but from the host perspective, only EXEC mode is observable.
 */
static int is_injection_mode_legal(void)
{
    int m = quasi88_get_mode();
...
/*
 * ASCII to KEY88 keycode translation
 * Returns KEY88_INVALID if not mappable
 */
static int ascii_to_key88(int c)
{
    /* Printable ASCII 32-126 maps directly */
    if (c >= 32 && c <= 126) {
	if (c >= 'A' && c <= 'Z')   return c;
	if (c >= 'a' && c <= 'z')   return c;
	if (c >= '0' && c <= '9')   return c;
...
/*
 * Count the total number of queue entries needed for a sequence.
 * Returns -1 if sequence is malformed (malformed escape, NULL char, unmappable).
 * Each key requires 2 entries (press + release).
 */
static int count_sequence_entries(const char *sequence)
{
	...
	if (*p == '<') {
	    ...
	    if (end == NULL) return -1;  /* Malformed: < without > */

	    /* Check escape content */
	    int len = (int)(end - p);
	    if (len == 3 && strncmp(p + 1, "CR", 2) == 0) {
		/* <CR> -> RETURN key */
		count += 2;  /* press + release */
	    } else if (len == 3 && strncmp(p + 1, "RET", 3) == 0) {
		count += 2;
	    } else if (len == 3 && strncmp(p + 1, "ENT", 3) == 0) {
		count += 2;
	    } else {
...
/*
 * Enqueue a pre-validated sequence.
 * Caller must ensure sequence is valid and fits in queue.
 * Returns number of entries enqueued.
 */
static int enqueue_validated_sequence(const char *sequence)
{
	...
	if (*p == '<') {
		...
		key_code = KEY88_RETURN;  /* Should not happen - already validated */
	    }
	    p = end + 1;
	} else {
	...
	keyinject_queue[keyinject_tail].press_flag = TRUE;
	...
	/* Enqueue release */
	keyinject_queue[keyinject_tail].key_code = key_code;
	keyinject_queue[keyinject_tail].press_flag = FALSE;
	keyinject_tail = (keyinject_tail + 1) % KEYINJECT_QUEUE_SIZE;
	keyinject_count++;
	total_enqueued++;
    }
...
/*
 * Parse and enqueue a key sequence atomically.
 * Supports: plain ASCII, <CR> for RETURN
 * Rejects: NULL chars, malformed escapes, unsupported escapes
 *
 * Atomic semantics: either the entire sequence is enqueued,
 * or nothing is enqueued (on overflow or invalid sequence).
 */
static int parse_and_enqueue(const char *sequence)
{
    if (sequence == NULL) return -2;

    /* Phase 1: Validate sequence and count required entries */
    int needed = count_sequence_entries(sequence);
    if (needed < 0) {
	return -2;  /* Invalid sequence */
    }

    if (needed == 0) {
	return 0;  /* Empty sequence is success */
    }

    /* Phase 2: Check if entire sequence fits */
    if (keyinject_count + needed > KEYINJECT_QUEUE_SIZE) {
	return -3;  /* Overflow - no partial enqueue */
    }

    /* Phase 3: Enqueue the validated sequence */
...
/****************************************************************************
 * Queued Input API Implementation
 *****************************************************************************/

/*
 * quasi88_keyinject - Queue a key sequence for later consumption
 *
 * Returns:
 *   0  Success (queued)
 *  -1  Bad mode (injection not allowed in current mode)
 *  -2  Bad sequence (malformed escape or NULL char)
 *  -3  Overflow (queue full, no partial enqueue)
 */
int quasi88_keyinject(const char *sequence)
{
    ...
    /* Parse and enqueue the sequence */
    return parse_and_enqueue(sequence);
}

/*
 * quasi88_keyinject_drain - Drain one key event from the queue
 *
 * Called during EXEC mode key processing to consume queued keys.
 * Returns TRUE if a key was drained and should be injected, FALSE if queue empty.
 *
 * Drain semantics:
 *   - One entry per call (press OR release, not both)
 *   - Queue entry is consumed (removed) when drained
 *   - FIFO ordering is preserved
 *   - Head pointer advances after successful drain
 *
 * The caller must inject the drained key via quasi88_key().
 *
 * Internal use only - called from keyboard_update() during EXEC mode.
 */
int quasi88_keyinject_drain(int *code_out, int *press_flag_out)
{
    ...
    *press_flag_out = keyinject_queue[keyinject_head].press_flag;
    ...
    return TRUE;  /* Key drained */
}

/*
 * quasi88_keyinject_reset - Reset/clear the key injection queue
 *
 * This flushes all pending key events and resets queue pointers.
 * 
 * Called on:
 *   - Emulator reset (emu_reset path) - flush pending input
 *   - Shutdown/exit paths - cleanup
 *   - Mode transition to MENU or QUIT - discard queued input
 *
 * Queue IS PRESERVED across:
 *   - MONITOR -> EXEC transition
 *   - EXEC -> MONITOR -> EXEC cycle
 *   - PAUSE -> EXEC transition
 */
...
void	keyboard_update(void)
{
	...
	/* Queued key injection - drain ONE key event per call for determinism.
	 * This ensures predictable timing and prevents burst injection.
	 * One full vframe update drains one queue entry. */
    if (quasi88_is_exec()) {
