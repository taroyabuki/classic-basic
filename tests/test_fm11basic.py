...
class FM11BasicRuntimeTests(unittest.TestCase):
    ...
    def test_collect_run_output_filters_mixed_punctuated_lx_noise_line(self) -> None:
        session = mock.Mock()
        session.copy_screen.side_effect = [
            "CBATCHBEGIN\nRUN\nSPACE OK\nHITS OK",
            'SPACE OK\nHITS OK\nLX   U                     ",      u P       C  x',
        ...
        self.assertEqual(lines, ["SPACE OK", "HITS OK", "SUM1 OK", "SUM2 OK"])
        self.assertEqual(
            emitted,
            [
                ["SPACE OK", "HITS OK"],
    ...
    def test_collect_run_output_waits_for_stable_mandelbrot_block(self) -> None:
        art = [
        ...
        self.assertEqual(lines, art)
        self.assertEqual(emitted, [art])

    def test_collect_run_output_reconstructs_mandelbrot_from_scrolling_history(self) -> None:
        art = [
            "000000011111111111111111122222233347E7AB322222111100000000000000000000000000000",
            "000001111111111111111122222222333557BF75433222211111000000000000000000000000000",
            "000111111111111111112222222233445C      643332222111110000000000000000000000000",
