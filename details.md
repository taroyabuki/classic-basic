...
file-run の実行時間は既定で無制限です。必要なときだけ `--timeout` を付けてください。`--timeout` は処理系の起動から batch 終了までの wall time 全体に効きます。環境変数で指定する場合は、N88-BASIC は `CLASSIC_BASIC_N88_BATCH_TIMEOUT`、MSX-BASIC は `CLASSIC_BASIC_MSX_BATCH_TIMEOUT`、GW-BASIC / QBasic は `CLASSIC_BASIC_DOSEMU_BATCH_TIMEOUT`、BASIC-80 は `CLASSIC_BASIC_RUNCPM_BATCH_TIMEOUT` を使えます。値は `90`、`2.5`、`90s`、`2m` のように指定できます。
...
| FM7-BASIC | MAME `fm77av` + `downloads/fm7basic/mame-roms` | 同じ | `mame-0.221-roms-merged/fm7.zip` 由来の FM-77AV 互換 ROM 群を MAME driver で起動 | file-load は typed input、batch は sub CPU console RAM を直接読む。既定は `fm77av` driver、`--driver fm7` は実験扱い | ROM 数値経路の確認に使える |
...
- direct mode の `@` は host 側で Enter に変換しているのではなく、original ROM がその文字を行編集の区切りとして扱う挙動をそのまま再現します。literal `@` を打てるようにする host 独自 workaround は入れていません。
- ローカル確認:
...
- emulator 本体は local `vendor/quasi88/` に同梱している official QUASI88 `0.7.3` ベースで、元の配布元は `https://www.eonet.ne.jp/~showtime/quasi88/download.html` です。repo 側で N88 batch 実行用の control bridge を追加しています。
...
- `setup/fm7basic.sh` は、まず MAME が無ければ apt で `mame` パッケージの導入を試み、その後、既存の FM-77AV ROM 群を見つければ `downloads/fm7basic/mame-roms/fm77av/` へコピーし、見つからなければ `mame-0.221-roms-merged/fm7.zip` を `downloads/fm7basic/fm7.zip` に取得して必要 ROM を抽出します。
...
- batch mode はホスト側 helper が headless interactive queue を流し、FM-77AV の sub CPU console RAM が `READY` に戻った時点で `PRINT "CBATCHBEGIN"` と `RUN` を送って出力行を回収します。`INPUT` などのプログラム入力待ちは未対応としてエラーにします。
...
- `setup/msxbasic.sh` は `mame-0.221-roms-merged/vg802020.zip` から `8020-20bios.rom` を抽出し、`TARGET_ROM_SHA256=1c85dac5536fa3ba6f2cb70deba02ff680b34ac6cc787d2977258bd663a99555` を確認して `downloads/msx/msx1-basic-bios.rom` と `downloads/msx/vg8020-20_basic-bios1.rom` に置くので、既定 ROM は固定されています。
...
- `run/qbasic.sh` の対話モードは full-screen IDE を端末へそのまま流さず、host 側の text shell として動きます。行番号付き入力は shell 側で保持し、`LIST` / `RUN` / `NEW` と direct statement を text-only で扱います。`run/qbasic.sh --file PROGRAM.bas` は line-numbered ASCII source を `RUNFILE.BAS` として stage した上で shell に読み込み、`RUN` のたびに real QBasic batch backend で実行します。EOF (`Ctrl-D`) または `SYSTEM` で終了します。
...
- text shell は full-screen の dosemu/QBasic 描画を端末へ出さないので、80x25 固定の端末サイズ制約には依存しません。
- custom `--command DOSCMD` を TTY で使う場合だけ legacy PTY bridge を通します。標準の `qbasic` 対話経路は text shell が source of truth です。
- **テストの注意点**:
  - 自動 test では `Ctrl-D` 終了を固定しておくこと
  - `test_qbasic_exec_smoke` は text shell から direct statement を流し、real QBasic batch backend の結果が返ることを自動検証する
- 結論:
...
- FM7-BASIC は「MAME fm77av + MAME fm7.zip-derived ROMs」
